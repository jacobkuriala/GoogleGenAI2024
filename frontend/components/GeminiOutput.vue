<template>
  ...
  >
    <div
      ...
      v-if="(output === '' || undefined || null) && !loading"
    >
      <NuxtImg
        src="/images/illustration.png"
        format="auto"
        sizes="70vw sm:70vw md:50vw lg:40vw"
      />
      <p class="w-1/2 text-slate-800 text-center text-xl font-normal">
        {{ placeholder }}
      </p>
    </div>
    <div
    ...
    >
      <div role="status">
        <svg
          aria-hidden="true"
          class="w-12 h-12 text-slate-800 animate-spin fill-corporate-500"
          viewBox="0 0 100 101"
          fill="none"
...
.output h1 {
  @apply self-stretch text-slate-400  text-2xl font-normal leading-8;
}
.output h2 {
  @apply text-slate-400 text-xl font-normal leading-6;
}
.output p {
